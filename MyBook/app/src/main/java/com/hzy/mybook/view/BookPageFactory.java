...
//    private int m_fontSize = 28;
    ...
    //0xffff9e85
    private int m_backColor = Color.rgb(200,177,129); // 背景颜色
//    private int m_backColor = Color.argb(12,200,177,129); // 背景颜色
    ...
    public BookPageFactory(Context context, int w, int h) {
        ...
        mPaint.setTextSize(textManager.getM_fontSize());//设置一个计算，计算文字的大小
        mPaint.setColor(m_textColor);
        mVisibleWidth = mWidth - marginWidth * 2;
        mVisibleHeight = mHeight - marginHeight * 2;//屏幕的高度-上下预留的距离
        mLineCount = (int) (mVisibleHeight / textManager.getM_fontSize()); // 可显示的行数:真正显示文本的行数
    }
    ...
    public void onDraw(Canvas c) {
        ...
        if (m_lines.size() > 0) {
//            m_book_bg = BitmapFactory.decodeResource(,R.drawable.fg_bg);
//            setBgBitmap(BitmapFactory.decodeResource(Resources.getSystem(), R.drawable.fg_bg));
            ...
            for (String strLine : m_lines) {
                y += textManager.getM_fontSize();//一次绘制一行
                c.drawText(strLine, marginWidth, y, mPaint);
                Log.d("MyBookActivity", strLine);
            }
        }
        //计算分页的内容，也是绘制在屏幕
        ...
        c.drawText(DateUtils.formatTime() + "  " + strPercent + "  test", 0, mHeight - 5, mPaint);
        // 修改SQList book的数据
